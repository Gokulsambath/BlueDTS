...
class SessionBuilder {
    ...
    async initIncoming(record, message) {
        const fqAddr = this.addr.toString();
        if (!await this.storage.isTrustedIdentity(fqAddr, message.identityKey.buffer)) {
    ...
    async initSession(isInitiator, ourEphemeralKey, ourSignedKey, theirIdentityPubKey,
        ...
        const a1 = curve.calculateAgreement(theirSignedPubKey, Buffer.from(ourIdentityKey.privKey));
        const a2 = curve.calculateAgreement(theirIdentityPubKey, Buffer.from(ourSignedKey.privKey));
        const a3 = curve.calculateAgreement(theirSignedPubKey, Buffer.from(ourSignedKey.privKey));
        if (isInitiator) {
        ...
        if (ourEphemeralKey && theirEphemeralPubKey) {
            const a4 = curve.calculateAgreement(theirEphemeralPubKey, Buffer.from(ourEphemeralKey.privKey));
