...
class SessionCipher {
    ...
    getRecord() {
        const record = this.storage.loadSession(this.addr.toString());
    ...
    async encrypt(data) {
        ...
        return await this.queueJob(async () => {
            ...
            if (this.storage.isTrustedIdentity(this.addr.id, remoteIdentityKey)) {
    ...
    async doDecryptWhisperMessage(messageBuffer, session) {
        ...
        macInput.set(Buffer.from(ourIdentityKey.pubKey), 33);
        macInput[33 * 2] = this._encodeTupleByte(VERSION, VERSION);
    ...
    maybeStepRatchet(session, remoteKey, previousCounter) {
        ...
        const prevCounter = session.getChain(Buffer.from(ratchet.ephemeralKeyPair.pubKey));
        if (prevCounter) {
    ...
    calculateRatchet(session, remoteKey, sending) {
        ...
        const sharedSecret = curve.calculateAgreement(remoteKey, Buffer.from(ratchet.ephemeralKeyPair.privKey));
